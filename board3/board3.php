...
<div id="mainWrapper">
  ...
  <div id="content">
	...
	<nav class="menu">
  	<h2>사장님게시판</h2>
  	<h4>사장님들의 창업 공략 및 운영 전략을 공유하는 게시판입니다.</h4>
	<hr>
      ...
      <thead>
		...
		if(isset($_GET['page'])){
          $page = $_GET['page'];
              ...
              $sql = mq("select * from board1 where boardtype=3");
        ...
        $sql2 = mq("select * from board1 where boardtype=3 order by idx desc limit $start_num, $list"); 
            while($board3 = $sql2->fetch_array())
            {
              ...
              { 
                //title이 30을 넘어서면 ...표시
                $title3=str_replace($board3["title"],mb_substr($board3["title"],0,25,"utf-8")."...",$board3["title"]);
              }
			//댓글 수 카운트
              $sql3 = mq("select * from reply1 where con_num='".$board3['idx']."'"); //reply테이블에서 con_num이 board의 idx와 같은 것을 선택
              $rep_count = mysqli_num_rows($sql3); //num_rows로 정수형태로 출력
        ?>
      <tbody>
        <tr>
          <td width="70"><?php echo $board3['idx']; ?></td>
          <td width="500"><a href="/wnateam/board/read.php?idx=<?php echo $board3["idx"];?>"><?php echo $title3;?><span class="re_ct">[<?php echo $rep_count; ?>]</span></a></td>
	...
	<br>
	<?php
		  $sql4 = mq("select count(*) from board1 where boardtype=3");
    ...
    <div>
        <?php
          ...
          { //만약 page가 1보다 크거나 같다면
            ...
            echo "<a href='https://wnateam-bieok.run.goorm.io/wnateam/board3/board3.php?page=1'> 처음 </a>"; //알니라면 처음글자에 1번페이지로 갈 수있게 링크
          }
          for($i=$block_start; $i<=$block_end; $i++){ 
            ...
            if($page == $i){ //만약 page가 $i와 같다면 
              ...
              echo "<a href='https://wnateam-bieok.run.goorm.io/wnateam/board3/board3.php?page=$i'> [$i] </a>"; //아니라면 $i
            }
          }
          if($page >= $total_page){ //만약 page가 페이지수보다 크거나 같다면
            echo "<a> 마지막 </a>"; //마지막 글자에 긁은 빨간색을 적용한다.
          }else{
            echo "<a href='https://wnateam-bieok.run.goorm.io/wnateam/board3/board3.php?page=$total_page'> 마지막 </a>"; //아니라면 마지막글자에 total_page를 링크한다.
          }
        ?>
    </div>
	...
	<?php }
      ...
      <a href="/wnateam/board3/write3.php"><button>글쓰기</button></a>
	<?php }?>
    </div>
    </nav>
	</section>	
