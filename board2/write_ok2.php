...
if(!$mkt_name){$mkt_name = NULL;}
$coor = $_GET['coor'];
if(!$coor){$coor = NULL;}
if($usertype == "B"){
	if($userid && $userpw && $title && $content){
    	$sql = mq("insert into board1(name, pw, title, content, date, hit, thumbup, boardtype, mkt_name, coor) values('".$userid."','".$userpw."','".$title."','".$content."','".$date."','0','0','2','".$mkt_name."','".$coor."')"); 
    	echo "<script>
    	alert('글쓰기 완료되었습니다.');
