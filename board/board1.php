...
<link href="Assets/styles/WriteStyle.css" rel="stylesheet" type="text/css">
</head>
<body>
<div id="mainWrapper">
  <header> 
      ...
      <thead>
		...
		if(isset($_GET['page'])){
          $page = $_GET['page'];
              ...
              $sql = mq("select * from board1 where boardtype=1");
        ...
        $sql2 = mq("select * from board1 where boardtype=1 order by idx desc limit $start_num, $list"); 
			...
			//댓글 수 카운트
              $sql3 = mq("select * from reply1 where con_num='".$board['idx']."'"); //reply테이블에서 con_num이 board의 idx와 같은 것을 선택
              $rep_count = mysqli_num_rows($sql3); //num_rows로 정수형태로 출력
        ?>
      <tbody>
        <tr>
          <td width="70"><?php echo $board['idx']; ?></td>
          <td width="500"><a href="/wnateam/board/read.php?idx=<?php echo $board["idx"];?>"><?php echo $title;?><span class="re_ct">[<?php echo $rep_count; ?>]</span></a></td>
    ...
    <div>
        <?php
          ...
          { //만약 page가 1보다 크거나 같다면
            ...
            echo "<a href='https://wnateam-bieok.run.goorm.io/wnateam/board/board1.php?page=1'> 처음 </a>"; //알니라면 처음글자에 1번페이지로 갈 수있게 링크
          }
          for($i=$block_start; $i<=$block_end; $i++){ 
            ...
            if($page == $i){ //만약 page가 $i와 같다면 
              ...
              echo "<a href='https://wnateam-bieok.run.goorm.io/wnateam/board/board1.php?page=$i'> [$i] </a>"; //아니라면 $i
            }
          }
          if($page >= $total_page){ //만약 page가 페이지수보다 크거나 같다면
            echo "<a> 마지막 </a>"; //마지막 글자에 긁은 빨간색을 적용한다.
          }else{
            echo "<a href='https://wnateam-bieok.run.goorm.io/wnateam/board/board1.php?page=$total_page'> 마지막 </a>"; //아니라면 마지막글자에 total_page를 링크한다.
          }
        ?>
    </div>
	<?php
		  $sql4 = mq("select count(*) from board1");
			$count = $sql4->fetch_array()[0];
			if($count==0) echo "<p>게시글이 없습니다.";
	?>
    <div style ="position: absolute; left : 85%; top : 1300px;">
      <a href="/wnateam/board/write1.php"><button>글쓰기</button></a>
    </div>
    </nav>
	</section>	
	</div>
